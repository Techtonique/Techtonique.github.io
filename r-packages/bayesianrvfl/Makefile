.PHONY: check clean coverage docs getwd initialize install load render setwd start test usegit
.DEFAULT_GOAL := help
...
initialize: setwd ## initialize: install packages devtools, usethis and rmarkdown
	Rscript -e "utils::install.packages(c('devtools', 'usethis', 'rmarkdown'), repos='https://cloud.r-project.org')"
